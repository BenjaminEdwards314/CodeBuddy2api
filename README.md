...
对比本仓库（`BenjaminEdwards314/CodeBuddy2api`）的最新提交：

- **有新版本**：提示最新 commit 与日期，并给出「查看更新内容」链接
  （仅在浏览器打开，不会自动下载或安装）。
- **已是最新 / 比本仓库更新**：明确提示，无需操作。
- **无法判断**：当本地 commit 未发布到任何远端时，会如实说明无法比较，
  而不会误报有新版本。

也可以直接到 [Releases](https://github.com/BenjaminEdwards314/CodeBuddy2api/releases)
下载打包好的 `.app`。
